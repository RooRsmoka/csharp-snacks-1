...
//Quando dovete concatenare molte stringhe,
...
//Dato un vettore di interi, costruito con numeri casuali, calcolare
//la media dei valori
//Il vettore deve contenere 10000 elementi

Random randomNum = new Random();
int[] vInt = new int[10000];
for (int i = 0; i < vInt.Length; i++)
    vInt[i] = randomNum.Next(1, 50);

double media = 0.0;
foreach(int v in vInt)
{
    media += v;
}

Console.WriteLine(media / vInt.Length);

int[] v10000 = Enumerable.Range(0, 10000).Select(n => randomNum.Next(0, 50)).ToArray();